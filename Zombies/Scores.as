package  {
	...
	public class Scores extends MovieClip {
		...
		public function Open(playerScore:Number, rounds:Number, kills:Number, accuracy:Number, duration:Number) {
			IsOpen = true;
			Mouse.show();
			score = playerScore;
			...
			var letters:Array = new Array("h", "m", "s");
			var time:Array = new Array(duration / 3600, (duration%3600) / 60, duration%3600); // h, m, s
			lblTime.text = "";
			for (var i in time)
				if (Math.floor(time[i]) > 0)
					lblTime.text += String(Math.floor(time[i])) + letters[i] + " ";
			
			lblRounds.text = String(rounds);
			lblZombies.text = String(kills);
